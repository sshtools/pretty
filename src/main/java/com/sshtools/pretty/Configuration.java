...
import com.sshtools.pretty.INISet.Scope;
...
public final class Configuration {
	...
	Configuration(Monitor monitor) {
		...
		iniSet = new INISet.Builder("pretty", monitor).
				withApp("pretty").
//				withDefault(Configuration.class, "Configuration.ini").
				withSchema(Configuration.class, "Configuration.schema.ini").
				build();
	...
	public <E extends Enum<E>> Handle bind(Class<E> clazz, SingleSelectionModel<E> observable, String key, String... path) {
		var sec = resolve(ini, path);
		var vuhndl = sec.onValueUpdate(ve -> {
			if(ve.key().equals(key)) {
